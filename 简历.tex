...
% ----- 页面设置 -----
\geometry{left=1.2cm, right=1.2cm, top=1.0cm, bottom=0.8cm} % 稍微收缩上下边距
\pagestyle{empty} 
...
\begin{center}
    ...
    \faEnvelope \ 1463967532@qq.com \quad
    \faMapMarker \ 籍贯：浙江台州 
\end{center}
...
\begin{itemize}[leftmargin=1.5em, noitemsep, topsep=1pt]
    ...
    \item 2. 通过\textbf{插值算法}实现\textbf{里程计 SLAM 融合定位}，采用 \textbf{map->odom} 表示码盘偏移，频率从10HZ提到\textbf{100HZ}
    \item 3. 基于点激光传感器完成 SLAM \textbf{初始位置的纠正算法}，纠正后因起始点导致的误差从 10-20cm 降到 \textbf{2cm} 内
...
{\kaishu 项目简介：面向 ROBOCON 高速机动赛场，完成舵轮底盘 MPC 轨迹跟踪控制}
\begin{itemize}[leftmargin=1.5em, noitemsep, topsep=1pt]
    \item \textbf{负责工作：} 1. 搭建 \textbf{MuJoCo + ROS2} 舵轮仿真链路，完成比赛地图构建、车体质量/转动惯量配置、关节响应时间调整，让仿真更贴近实际表现。
    \item 2. 基于 \textbf{do-mpc + CasADi} 实现 MPC 路径跟踪控制器（状态量 $[x,y,\theta]$，控制输入 $[v,\alpha,\omega_z]$，约束状态$[dx,dy,domega]$）。配置位置软约束，输入软约束与最大速度硬约束。
...
\begin{itemize}[leftmargin=1.5em, noitemsep, topsep=1pt]
    ...
    \item \textbf{项目成果：} 完成\textbf{国家级实验平台}效果验证，在大湍流下实现\textbf{多 30\% 风能捕获率}
\end{itemize}
...
\begin{itemize}[leftmargin=1.5em, nosep, topsep=2pt]
    \item \textbf{编程与系统} C/C++，PYTHON，CMAKE，FreeRTOS，Linux，Docker，Git
    \item \textbf{软件/库} ROS2，OpenCV，Vscode，Keil，Nav2
\end{itemize}
\end{document}